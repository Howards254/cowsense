  ...
  head: () => ({
    meta: [
      { title: "Dashboard · CowSense AI" },
      {
        name: "description",
        content:
          "Turning farmer data into action — AI-powered farmer intelligence for extension agents.",
      },
    ],
  }),
...
function DashboardPage() {
  ...
  const { data: farmerTrend } = useQuery({
    queryKey: ["dashboard", "farmerTrend"],
    queryFn: () => dashboardService.getFarmerTrend(),
  });
  const { data: inputTrend } = useQuery({
    queryKey: ["dashboard", "inputTrend"],
    queryFn: () => dashboardService.getInputDemandTrend(),
  });
  const { data: countyDemand } = useQuery({
    queryKey: ["dashboard", "countyDemand"],
    queryFn: () => dashboardService.getCountyDemand(),
  });
  const { data: followUps = [] } = useQuery({
    queryKey: ["followups"],
    queryFn: () => followupService.list(),
  });
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={inputTrend}>
            ...
            <Area
              ...
              dataKey="silage"
              stroke="hsl(178 55% 35%)"
              fill="hsl(178 55% 35% / 0.25)"
            />
            <Area
              type="monotone"
              dataKey="dairyMeal"
              stroke="hsl(85 70% 45%)"
              fill="hsl(85 70% 45% / 0.25)"
            />
            <Area
              type="monotone"
              dataKey="vaccines"
              stroke="hsl(35 85% 55%)"
              fill="hsl(35 85% 55% / 0.25)"
            />
          </AreaChart>
        </ResponsiveContainer>
      </ChartCard>
        ...
        <div className="mt-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            {weather.slice(0, 6).map((w) => (
              <div
                key={w.county}
                className="rounded-xl border border-border bg-card p-3 shadow-sm text-center"
              >
          ...
          <ul className="divide-y divide-border">
            {todays.map((f) => (
              <li key={f.id} className="p-4 flex items-center justify-between gap-3">
                ...
                <span
                  className={`text-xs shrink-0 ${f.status === "overdue" ? "text-destructive" : "text-muted-foreground"}`}
                >
            ...
            {urgent.map((f) => (
              <li key={f.id} className="p-4">
                ...
                <div className="text-xs text-muted-foreground mt-1">
                  {f.county} · {f.issues.length} open issues
                </div>
              </li>
            ))}
          </ul>
